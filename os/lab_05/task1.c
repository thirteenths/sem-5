//Написать программу, реализующую задачу «Производство-
//потребление» по алгоритму Э. Дейкстры с тремя семафорами: двумя
//считающими и одним бинарным. В программе должно быть создано не
//менее трех процессов -производителей и трех процессов –
//потребителей. Необходимо обеспечить случайные задержки
//выполнения созданных процессов. В программе для взаимодействия
//производителей и потребителей буфер создается в разделяемом
//сегменте. Следует обратить внимание на то, чтобы не работать с
//одиночной переменной, а работать именно с буфером, состоящим их N
//ячеек по алгоритму. Производители в ячейки буфера записывают буквы
//алфавита по порядку (fifo). Потребители считывают символы из
//доступной ячейки. После считывания буквы из ячейки следующий
//потребитель может взять букву только из следующей ячейки.
